...
# Обработка Оборотов и остатков по активам
##############################################################################
...
class BalanceAssets(ModelSQL, ModelView):
    ...
    def post_balance(self, values=None):
        ...
        if not values or not values.get('analytic', False):
            return {}
        assets_period_obj = self.pool.get('ekd.balances.assets.period')
        assets_period_create = assets_period_obj.create
        res={}
        need_actived=False
        search_find = {}
        search_not_find = {}
        ...
        Удаление из остатка и оборота в учетном регистре счета
            values = {
                ...
                'analytic': [{
                    ...
                    'level': Уровень аналитики,
                    'model_ref': модель и ID,
                    'balance_party': ID Assets Account (Party),
