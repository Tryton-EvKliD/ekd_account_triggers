...
class BalanceAnalytic(ModelSQL, ModelView):
    ...
    def __init__(self):
        super(BalanceAnalytic, self).__init__()
    ...
    def post_balance(self, values=None):
        ...
        '''
        if not values or not values.get('analytic', False):
            return {}
        analytic_period_obj = self.pool.get('ekd.balances.analytic.period')
        #analytic_year_obj = self.pool.get('ekd.balances.party.year')
        analytic_period_create = analytic_period_obj.create
        res={}
        need_actived=False
        search_find = {}
        search_not_find = {}
        ...
        if search_find:
            ...
            if amount_ids:
                ...
                if values.get('debit'):
                    cr.execute("UPDATE ekd_balances_analytic_period "\
        ...
        if search_not_find:
            ...
            for level in level_not_find:
                ...
                #raise Exception(str(search_not_find.get(level_sort[level])))
                analytic_id = self.create(search_not_find.get(level_sort[level]))
                if values.get('debit'):
                    amount_id = analytic_period_create({
                            'account': analytic_id,
                            'period': values.get('period').id,
                            'debit': values.get('debit'),
                            })
    ...
    def post_cancel(self, values=None):
        ...
        cr = Transaction().cursor
        if values.get('debit'):
            cr.execute("UPDATE "+party_period_obj._table+\
                    " SET debit = debit-%s,"\
                        " write_date='%s' "%\
                        (values.get('debit'), datetime.datetime.now())+\
                    " WHERE id in("+",".join(str(x.ref_period) \
                        for x in values.get('analytic'))+")")
        elif values.get('credit'):
            cr.execute("UPDATE "+party_period_obj._table+\
                    " SET credit = credit-%s, "\
                        " write_date='%s' "%\
